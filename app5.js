/* =========================
   ...
   Унифицированный, с FIX: не затирать inline-фон/opacity,
   + ползунок «Прозрачность окон (.glass)»
   ========================= */
...
/* FIX: НЕ трогаем inline backgroundImage и opacity, если ключей в LS нет */
function applySection(id){
  const el = byId('bg-' + id);
  if(!el) return;

  const bgStored    = localStorage.getItem(bgKey(id));     // null | "" | "file" | "none"
  const alphaStored = localStorage.getItem(alphaKey(id));  // null | "0..100"

  // --- ФОН ---
  if (bgStored === null || bgStored.trim() === '') {
    // Ключа нет ИЛИ пустой → оставляем inline backgroundImage из HTML
  } else if (bgStored === 'none') {
    el.style.backgroundImage = '';
  } else {
    const file = basename(bgStored);
  ...
  // --- ПРОЗРАЧНОСТЬ ---
  if (alphaStored !== null) {
    const alpha = Math.max(0, Math.min(100, Number(alphaStored) || 0));
    el.style.opacity = String(alpha / 100);
  }
}
...
function initSettingsPanel(){
  ...
  function sync(){
    ...
    urlIn.value = stored ? basename(stored) : '';

    // прозрачность раздела
    const aStored = localStorage.getItem(alphaKey(id));
    const a = (aStored===null) ? '28' : String(Number(aStored)); // UI значение
    alpha.value = a; alphaVal.textContent = a + '%';

    // Прозрачность окон (.glass)
  ...
  if(reset)  reset.addEventListener('click',()=>{
    ...
    localStorage.removeItem(alphaKey(id));         // ключ исчезает => opacity НЕ трогаем
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  IDS.forEach(applySection);      // применяем фоны с учётом FIX (inline не затирается)
